...
M.misc = function()
    ...
    local function optional_mappings()
        ...
        if not nvChad_options.copy_cut then map({"n", "v"}, "x", '"_x') end

        -- don't yank text on delete ( dd )
        if not nvChad_options.copy_del then map({"n", "v"}, "d", '"_d') end
...
M.lspconfig = function()
    ...
    map("n", m.list_workspace_folders,
...
M.telescope = function()
    ...
    map("n", m.find_git_files, ":GitFiles <CR>")
